...
$pdf->AddPage();
	...
	$pdf->SetFont('Times','',10);
		$pdf->Cell(110,10,$cliente_cons->nombre1.' '.$cliente_cons->nombre2.' '.$cliente_cons->apellido1.' '.$cliente_cons->apellido2,0,0);
		$pdf->Cell(40,10,$remision->fecha_rem,0,0);
	...
	$pdf->SetFont('Times','',10);
		...
		$pdf->Cell(90,10,$cont_cons->nombre,0,0);
	...
	$pdf->SetFont('Times','B',10);
		...
		$pdf->Ln(5);
	$pdf->SetFont('Times','',10);	
		$pdf->Cell(120,10,$cliente_cons->direccion,0,0);
...
for($i=0; $i<count($con_eq); $i++){
	...
	$eq=$equipo_rem[0];
	$pdf->Cell(25,6,$eq->referencia,1,0); //referencia
	$pdf->Cell(95,6,$eq->nombre,1,0);//nombre
	$pdf->Cell(20,6,$id_equipo->cantidad,1,0);//cantidad
	if($eq->valor_dia != '0'){
		$pdf->Cell(25,6,$eq->valor_dia,1,0);//valor unitario/día
	}
	$total=$id_equipo->cantidad*$eq->valor_dia;
	$pdf->Cell(27,6,$total,1,1);//valor alquiler
}
for($j=0; $j<count($eqsubrem); $j++){
	$equipos1=$eqsubrem[$j];
	$equipo_sub_rem = $entradasub->buscarequipoEntrada('id', $equipos1->id_equipo_sub);
	$eqsub=$equipo_sub_rem[0];
	$pdf->Cell(25,6,'',1,0); //referencia
	$pdf->Cell(95,6,$eqsub->nombre,1,0);//nombre
	$pdf->Cell(20,6,$eqsub->cantidad,1,0);//cantidad
	$pdf->Cell(25,6,'',1,0);//valor unitario/día
	$pdf->Cell(27,6,'',1,1);//valor alquiler
}
