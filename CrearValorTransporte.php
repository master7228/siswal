...
if ($_SESSION["autenticado"] == "1") {
    ...
    if ($_POST['tarea'] == 'guardar') {
		...
		if(!$ConsultarValorTransporte){
			$guardar = $valortransporte->guardarValorTransporte();
			if ($guardar) {
				$consul = $valortransporte->buscarValorTransporteDos($_POST['ciudad'],$_POST['valor']);
				$tran=$consul[0];
				$registro= new Registro('',$_SESSION['id_usuario'],'CREAR VALOR TRANSPORTE',$tran->id,$tran->valor,$fecha);
				$guardar_registro = $registro->guardarRegistro();
				echo "<script>alert ('El Valor Transporte fue creado con exito');</script>";
				echo "<script>window.location.href='MenuValorTransporte.php';</script>";
			} else {
				echo "<script>alert ('No se pudo crear el valor transporte, por favor intenta de nuevo');</script>";
				echo "<script>window.location.href='CrearValorTransporte.php';</script>";
			}
		}else{
...
<title>Crear Valor Transporte</title>
  ...
  <tr>
    <td align="center" valign="top"><form action="" method="post" name="form1" class="FormGeneral" id="form1">
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td align="center" valign="middle"><strong>CREAR VALOR TRANSPORTE</strong></td>
        </tr>
      </table>
            ...
            <tr>
              <td height="132" align="center" valign="top"><table width="718" border="0" align="center" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  ...
                  <td width="345" align="center"><input name="ciudad" type="text" id="ciudad" size="30" maxlength="25" class="campos_mayus" onKeyPress="javascript:return Letras(event)"/>                    <input name="tarea" type="hidden" id="tarea" class="campos_mayus" /></td>
                  <td width="303" align="center"><input name="valor" type="text" id="valor" size="30" maxlength="12" class="campos_mayus" onKeyPress="javascript:return Numeros(event)"/></td>
                  </tr>
                <tr>
                  <td align="center" valign="middle">&nbsp;</td>
                  <td align="left">&nbsp;</td>
