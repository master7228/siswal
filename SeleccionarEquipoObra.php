<?php
 ...
 if($_SESSION["autenticado"] == "1"){
	 ...
	 if(!empty($_POST['select']) && !empty($_POST['palabra']) ){
		 $equipo = new Equipo("","","","","","","","","","","","","","","","","","","","","","","","","","","");
		 $resp = $equipo->buscarEquipoSelDevolucion($_POST['select'],$_POST['palabra'],$_SESSION['idcliente'],$_SESSION['idobra']);
	 ...
	 if(!empty($_POST['tarea']) && $_POST['tarea']=='seleccionar'){
		if(empty($_SESSION['conta'])){
			...
			unset($_SESSION['cont']);
			/*cho "<script>window.opener.location.reload(); window.close();</script>";*/
		 }else{
			...
			for($i=0; $i<$_SESSION['cont']; $i++){
				if (!empty($_POST['seleccionar'.$i.''])){
					...
					$conta ++;
					$equipo_sel = new Equipo('','','','','','','','','','','','','','','','','','','','','','','','','');
					$resp_equipo_sel = $equipo_sel->cambiarEstadoEquipo('B',$_POST['seleccionar'.$i.''],$_SESSION['id_usuario']);
				}
			}
			$_SESSION['conta']= $conta;
			$_SESSION['id_equipos']=serialize($id_equipos);
...
<title>Buscar Equipo en Obra</title>
